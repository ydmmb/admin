...
public class RealmHelper implements DBHelper {
    ...
    public static final String DB_NAME = "universe.realm";
    ...
    public boolean queryCaesarId(String id) {
        RealmResults<Caesar> results = getRealm().where(Caesar.class).findAll();
        for (Caesar item : results) {
            if (item.getId().equals(id)) {
                return true;
            }
        }
    ...
    public void deleteCaesar(String id) {
        Caesar data = getRealm().where(Caesar.class).equalTo("id", id).findFirst();
        getRealm().beginTransaction();
        data.deleteFromRealm();
        getRealm().commitTransaction();
    }
    ...
    public void deleteAllCaesar() {
        getRealm().beginTransaction();
        getRealm().delete(Caesar.class);
        getRealm().commitTransaction();
    }

    public List<Caesar> getCaesarList() {
        //使用findAllSort ,先findAll再result.sort排序
        RealmResults<Caesar> results = getRealm().where(Caesar.class).findAllSorted("time", Sort.DESCENDING);
        return getRealm().copyFromRealm(results);
    }
